...
現有 runtime 支援 `Sucheng`、`New Sucheng`、`Cangjie`、`Pinyin`、automatic raw English pass-through 同 optional local English spelling suggestions。英文 spelling suggestions 作為 optional candidate provider，可以接在 raw English path 或混入一般候選頁，不改變現有英文原文 commit path。
...
English spelling suggestions 接在 current alphabetic token classifier 之後：

```text
Alphabetic token
  -> token classifier
  -> local spelling suggestion provider
  -> raw-English candidate UI or mixed Chinese/spelling candidate UI
  -> user-selected suggestion or original typed word
```

呢條 path 不得自動替換 typed word。
...
Engine router 不負責 English spelling suggestions。spelling suggestions 由 input controller 在候選 UI 前合併，並以 preference 控制。
...
Spelling suggestions 只可在 `RawEnglish` / `CandidateVisible` 之間加入 candidate rows；mixed candidate page 必須保留 primary Chinese candidate order，不可加入自動替換 transition。
...
Spelling suggestion provider 如果初始化失敗：
...
### Local English Spelling Provider

優點：

- 使用 macOS `NSSpellChecker` 可做到 spelling suggestions 而不送雲端。
- 不需要 bundle 大型英文詞庫或抽取 Apple dictionary 資料。
- 可用同一 candidate UI 表示「原文」同「建議拼法」。

缺點：

- 要避免干擾 raw English pass-through。
- suggestion quality 會跟隨 macOS 版本和用戶 spelling dictionary。
- repeated lookup 需要 cache，避免同一 token 重複打到系統 spell checker。
