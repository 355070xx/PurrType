...
Public builds 向 macOS 註冊一個 `PurrType` input source；速成、新速成、倉頡同拼音係 app 內部模式，由 PurrType menu、快捷鍵同 PurrType Preferences 切換。
...
- optional local English spelling suggestions via macOS `NSSpellChecker`

### Future Product

- Advanced Dictionary Manager for custom allow/block words.
...
- 不送出雲端查詢。
- suggestion lookup 使用 macOS `NSSpellChecker` documented API，不 bundle 或抽取 Apple dictionary 資料。
- repeated token lookup 應 cache，避免同一 token 重複打到 spell checker。
